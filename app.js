...
app.post('/api/addNewBill', async (req, res) => {
...
app.get('/api/getFarsanCustData', authenticateToken, async (req, res) => {
    try {
        const [result] = await pool.query('SELECT * FROM farsancustomers');
        res.json(result);
    } catch (err) {
        console.error(err);
...
app.post('/api/addFarsanCustomer', authenticateToken, async (req, res) => { 
    try {
        const [result] = await pool.query('INSERT INTO farsancustomers SET ?', [req.body]);
        res.json({ status: 'success', insertId: result.insertId });
    } catch (err) {
        console.error(err);
...
app.post('/api/updateFarsanCustomer', authenticateToken, async (req, res) => {      
    try {
         const [result] = await pool.query(
            'UPDATE farsancustomers SET name = ?, cadd = ?, area = ?, mobileno = ?, joindate = ?   WHERE cno = ?', 
            [req.body.name, req.body.cadd, req.body.area, req.body.mobileno, req.body.joindate, req.body.cno]
        );
        res.json(result);
    } catch (err) {
        console.error(err);
...
app.get('/api/getCustBalance', authenticateToken, async (req, res) => { 
    try {
        const [result] = await pool.query(
            "select * from farsanentry where cno='"+req.query.cno+"' ORDER BY id DESC LIMIT 1"
        );
        res.json(result);
    } catch (err) {
        console.error(err);
...
app.post('/api/addFarsanCustEntry', authenticateToken, async (req, res) => { 
    try {
        const [result] = await pool.query('INSERT INTO farsanentry SET ?', [req.body]);
        res.json({ status: 'success', insertId: result.insertId });
    } catch (err) {
        console.error(err);
...
app.get('/api/billEntryDate', authenticateToken, async (req, res) => {  
    try {
        const [result] = await pool.query(
            "SELECT * FROM farsanentry where trandate='"+req.query.trandate+"' ORDER BY id DESC"
        );
        res.json(result);
    } catch (err) {
        console.error(err);
...
app.get('/api/getCustBillLastNilBalRecord', authenticateToken, async (req, res) => { 
    try {
        const [result] = await pool.query(
            "select * from farsanentry where cno='"+req.query.cno+"' and balance = 0 ORDER BY id DESC LIMIT 1"
        );
        res.json(result);
    } catch (err) {
        console.error(err);
...
app.get('/api/getCustBillLastNilBalRecordBalZeroNotFound', authenticateToken, async (req, res) => { 
    try {
        const [result] = await pool.query(
            "select * from farsanentry where cno='"+req.query.cno+"' LIMIT 1"
        );
        res.json(result);
    } catch (err) {
        console.error(err);
...
app.get('/api/getFarsanCustDueRecords', authenticateToken, async (req, res) => { 
    try {
        const [result] = await pool.query(
            "select * from farsanentry where id >= '"+req.query.id+"' and  cno = '"+req.query.cno+"' ORDER BY id DESC"
        );
        res.json(result);
    } catch (err) {
        console.error(err);
...
app.get('/api/searchFarsanCustBills', authenticateToken, async (req, res) => {
    try {
        ...
        const [results] = await pool.query(
            "SELECT * FROM farsanentry where cno='"+cno+"' and packeddate >= '"+startDate+"' and packeddate <= '"+endDate+"' ORDER BY id DESC"
        );
        res.json(results);
    } catch (err) {
        console.error(err);
...
app.get('/api/getYearlyProductsSale', authenticateToken, async (req, res) => {
    try {
        ...
        const query = "SELECT * FROM farsanentry where cno='"+cno+"' and packeddate >= '"+startDate+"' and packeddate <= '"+endDate+"' ORDER BY id DESC";
        const [results] = await pool.query(query);
        res.json(results);
    } catch (err) {
        console.error(err);
